    ...
    <div style={{ display: 'flex', gap: '6px', margin: '6px 0' }}>
      {[1, 2, 3, 4, 5].map(i => (
        <button key={i} type="button" onClick={() => onChange(i)} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(0)}
          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', fontSize: '28px', color: (hover || wartosc) >= i ? '#A05C5C' : '#ddd', transition: 'color 0.15s' }}>★</button>
      ))}
    </div>
  );
}
...
function EkranAnkieta({ kursant, zjazdy, user }: { kursant: Kursant | null; zjazdy: Zjazd[]; user: User }) {
  ...
  const [wysylanie, setWysylanie] = useState(false);
  const [sukces, setSukces] = useState(false);
  const [bladWysylania, setBladWysylania] = useState('');  // ← POPRAWKA: obsługa błędów
  const [krok, setKrok] = useState(1);
  const [odpowiedzi, setOdpowiedzi] = useState<OdpowiedziAnkiety>({
    zadowolenie: 0, wiedza_wzrosla: '', zajecia_teoretyczne: 0, zajecia_rysunek: 0,
    zajecia_programy: 0, zakres_tematyczny: 0, org_czas: 0, org_miejsce: 0,
    org_baza: 0, org_materialy: 0, org_kadra: 0, org_dostosowanie: 0,
    stopien_oczekiwan: 0, ocena_ogolna: 0, przydatne_informacje: '', uzasadnienie_zle: '',
    inne_uwagi: '', nps: '', plec: '', wyksztalcenie: '', wiek: '',
  });

  const pokazUzasadnienie = [odpowiedzi.org_czas, odpowiedzi.org_miejsce, odpowiedzi.org_baza, odpowiedzi.org_materialy, odpowiedzi.org_kadra, odpowiedzi.org_dostosowanie].some(v => v > 0 && v <= 2);
  const ostatniZjazd = zjazdy.length > 0 ? zjazdy[zjazdy.length - 1] : null;
  const kursZakonczony = ostatniZjazd?.status === 'zakonczony';

  useEffect(() => {
    if (!user || !kursant?.grupa_id) return;
    supabase.from('ankiety').select('id').eq('grupa_id', kursant.grupa_id).eq('user_id', user.id).maybeSingle()
      .then(({ data }) => setWypelniona(!!data));
  }, [user, kursant]);
  ...
  // ← POPRAWKA: wyslij z obsługą błędów
  async function wyslij() {
    setWysylanie(true);
    setBladWysylania('');
    const { error } = await supabase.from('ankiety').insert([{
    ...
    if (!error) {
      ...
      setBladWysylania('Błąd wysyłania: ' + error.message);
  ...
  if (!kursZakonczony) {
    return (
      <div style={{ padding: '24px', textAlign: 'center' }}>
        <div style={{ fontSize: '48px', marginBottom: '16px' }}>🔒</div>
        <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '22px', marginBottom: '12px' }}>Ankieta niedostępna</h2>
        <p style={{ color: 'var(--text-muted)', fontSize: '14px', lineHeight: '1.6' }}>Ankieta oceniająca kurs zostanie odblokowana po zakończeniu ostatniego zjazdu Twojej grupy.</p>
        {ostatniZjazd && <div style={{ marginTop: '20px', background: 'var(--surface)', borderRadius: '12px', padding: '16px', border: '0.5px solid var(--border)' }}><p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Ostatni zjazd:</p><p style={{ fontWeight: '600', color: 'var(--brand)', marginTop: '4px' }}>Zjazd {ostatniZjazd.nr} — {ostatniZjazd.daty}</p></div>}
      </div>
    );
  }
  ...
  if (wypelniona || sukces) {
    return (
      <div style={{ padding: '24px', textAlign: 'center' }}>
        <div style={{ fontSize: '56px', marginBottom: '16px' }}>🎉</div>
        <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '24px', marginBottom: '12px' }}>Dziękujemy!</h2>
        <p style={{ color: 'var(--text-muted)', fontSize: '14px', lineHeight: '1.6' }}>Twoja opinia została zapisana. Dziękujemy za udział w kursie i za poświęcony czas.</p>
        <div style={{ marginTop: '24px', background: 'var(--surface)', borderRadius: '12px', padding: '16px', border: '0.5px solid var(--border)' }}>
          <p style={{ fontSize: '13px', color: 'var(--brand)', fontWeight: '600' }}>On-Arch Barbara Szczęsna-Dyńska</p>
          <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>info@on-arch.pl | +48 533 718 412</p>
        </div>
  ...
  const sekcjaTytul = (t: string) => <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '18px', color: 'var(--brand)', margin: '20px 0 8px' }}>{t}</h3>;
  const pytanieGwiazdki = (label: string, pole: keyof OdpowiedziAnkiety) => (
    <div style={{ marginBottom: '16px' }}>
      <p style={{ fontSize: '13px', color: 'var(--text)', lineHeight: '1.5', marginBottom: '4px' }}>{label}</p>
      <GwiazdkiOcena wartosc={odpowiedzi[pole] as number} onChange={v => ustaw(pole, v)} />
  ...
  const textarea = (label: string, pole: keyof OdpowiedziAnkiety, opcjonalne = false) => (
    <div className="login-field" style={{ marginBottom: '12px' }}>
      <label style={{ fontSize: '13px' }}>{label}{opcjonalne && <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}> (opcjonalnie)</span>}</label>
      <textarea value={odpowiedzi[pole] as string} onChange={e => ustaw(pole, e.target.value)} rows={3} style={{ resize: 'vertical', fontSize: '13px' }} />
    </div>
  );
  const krokowLacznie = 4;
  ...
  return (
    <div style={{ paddingBottom: '16px' }}>
      <div style={{ background: 'var(--brand)', color: 'white', padding: '16px 20px', borderRadius: '0 0 16px 16px', marginBottom: '4px' }}>
        ...
        <p style={{ fontSize: '12px', opacity: 0.85 }}>Odpowiedzi są anonimowe • Krok {krok} z {krokowLacznie}</p>
        <div style={{ display: 'flex', gap: '4px', marginTop: '10px' }}>
          {Array.from({ length: krokowLacznie }).map((_, i) => <div key={i} style={{ flex: 1, height: '3px', borderRadius: '2px', background: i < krok ? 'white' : 'rgba(255,255,255,0.3)' }} />)}
        </div>
      </div>
      <div style={{ padding: '8px 20px' }}>
        {krok === 1 && (
          <>
            ...
            {pytanieGwiazdki('1. Czy jest Pan/Pani zadowolony/a ze szkolenia?', 'zadowolenie')}
            <div style={{ marginBottom: '16px' }}>
              <p style={{ fontSize: '13px', color: 'var(--text)', lineHeight: '1.5', marginBottom: '8px' }}>2. Czy czujesz, że Twoja wiedza wzrosła?</p>
              <div style={{ display: 'flex', gap: '8px' }}>
                {(['Tak', 'Trochę', 'Nie'] as const).map(opcja => (
                  <button key={opcja} type="button" onClick={() => ustaw('wiedza_wzrosla', opcja)} style={{ flex: 1, padding: '10px 4px', borderRadius: '10px', fontSize: '13px', cursor: 'pointer', fontWeight: odpowiedzi.wiedza_wzrosla === opcja ? '600' : '400', background: odpowiedzi.wiedza_wzrosla === opcja ? '#A05C5C' : 'var(--surface)', color: odpowiedzi.wiedza_wzrosla === opcja ? 'white' : 'var(--text)', border: odpowiedzi.wiedza_wzrosla === opcja ? 'none' : '0.5px solid var(--border)', transition: 'all 0.15s' }}>{opcja}</button>
                ))}
              </div>
            </div>
            {sekcjaTytul('Prowadzenie zajęć')}
        ...
        {krok === 2 && (
          <>
            ...
            {pytanieGwiazdki('5a. Czas trwania szkolenia', 'org_czas')}
            <div style={{ marginBottom: '16px' }}>
              <p style={{ fontSize: '13px', color: 'var(--text)', lineHeight: '1.5', marginBottom: '4px' }}>5b. Miejsce szkolenia <span style={{ fontSize: '11px', color: 'var(--text-muted)', fontStyle: 'italic' }}>(nie dotyczy kursu online)</span></p>
              <GwiazdkiOcena wartosc={odpowiedzi.org_miejsce} onChange={v => ustaw('org_miejsce', v)} />
            </div>
            {pytanieGwiazdki('5c. Baza dydaktyczna (lokal, sprzęt)', 'org_baza')}
            {pytanieGwiazdki('5d. Jakość i przydatność materiałów szkoleniowych', 'org_materialy')}
            {pytanieGwiazdki('5e. Przygotowanie zawodowe kadry dydaktycznej', 'org_kadra')}
            {pytanieGwiazdki('5f. Dostosowanie poziomu zajęć do potrzeb grupy', 'org_dostosowanie')}
            {pokazUzasadnienie && <div style={{ marginBottom: '16px', padding: '12px', background: '#fff8f8', borderRadius: '10px', border: '1px solid #f5c0c0' }}>{textarea('Proszę o uzasadnienie niskiej oceny:', 'uzasadnienie_zle', true)}</div>}
          </>
        )}
        {krok === 3 && (
          <>
            ...
            {pytanieGwiazdki('Ogólna ocena szkolenia', 'ocena_ogolna')}
            <div style={{ marginBottom: '16px' }}>
              <p style={{ fontSize: '13px', color: 'var(--text)', lineHeight: '1.5', marginBottom: '8px' }}>7. Czy poleciłabyś/poleciłbyś ten kurs znajomym?</p>
              <div style={{ display: 'flex', gap: '8px' }}>
                {(['Tak', 'Może', 'Nie'] as const).map(opcja => (
                  <button key={opcja} type="button" onClick={() => ustaw('nps', opcja)} style={{ flex: 1, padding: '10px 4px', borderRadius: '10px', fontSize: '13px', cursor: 'pointer', fontWeight: odpowiedzi.nps === opcja ? '600' : '400', background: odpowiedzi.nps === opcja ? (opcja === 'Tak' ? '#4a7c59' : opcja === 'Nie' ? '#A05C5C' : '#7a6a3a') : 'var(--surface)', color: odpowiedzi.nps === opcja ? 'white' : 'var(--text)', border: odpowiedzi.nps === opcja ? 'none' : '0.5px solid var(--border)', transition: 'all 0.15s' }}>{opcja}</button>
                ))}
              </div>
            </div>
            {textarea('Które z przekazywanych informacji uważa Pan/Pani za najbardziej przydatne?', 'przydatne_informacje', true)}
        ...
        {krok === 4 && (
          <>
            {sekcjaTytul('Metryczka')}
            <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '16px' }}>Dane służą wyłącznie do celów statystycznych i nie pozwalają na identyfikację osoby.</p>
            {select('Płeć', 'plec', ['Kobieta', 'Mężczyzna', 'Inne', 'Wolę nie podawać'])}
            {select('Wykształcenie', 'wyksztalcenie', ['Podstawowe', 'Zawodowe', 'Średnie', 'Wyższe licencjackie', 'Wyższe magisterskie', 'Doktorat lub wyższe'])}
            {select('Wiek', 'wiek', ['18–24', '25–34', '35–44', '45–54', '55–64', '65+'])}
          </>
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: '24px' }}>
          {krok > 1 && <button onClick={() => setKrok(k => k - 1)} style={{ flex: 1, padding: '14px', borderRadius: '12px', border: '0.5px solid var(--brand)', background: 'white', color: 'var(--brand)', fontSize: '15px', cursor: 'pointer', fontWeight: '600' }}>← Wstecz</button>}
          {krok < krokowLacznie
            ? <button onClick={() => setKrok(k => k + 1)} className="login-btn" style={{ flex: 1 }}>Dalej →</button>
            : <button onClick={wyslij} className="login-btn" style={{ flex: 1 }} disabled={wysylanie}>{wysylanie ? 'Wysyłanie...' : 'Wyślij ankietę ✓'}</button>
          }
        </div>
        {/* ← POPRAWKA: wyświetlanie błędu */}
        {bladWysylania && (
          <div style={{ marginTop: '10px', padding: '10px 14px', background: '#ffebee', borderRadius: '10px', fontSize: '12px', color: '#c62828', lineHeight: 1.6 }}>
            ⚠ {bladWysylania}
          </div>
