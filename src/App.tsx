...
function urlBase64ToUint8Array(base64String: string) {
  ...
  function PytaniaProwadzacy({ user, zjazdy, grupy, kursant, prowadzacyImie }: { user: User; zjazdy: Zjazd[]; grupy: Grupa[]; kursant: Kursant | null; prowadzacyImie: string }) {
    ...
    // Unikalne tematy w kolejności zjazdów
    const tematy = [...new Map(zjazdyGrupy.map(z => [z.tematy, z])).values()];
    ...
    useEffect(() => {
      ...
      const channel = supabase.channel(`pytania-tablica-${wybranaGrupa}`)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'pytania_do_zjazdu',
    ...
    async function dodajPytanie(temat: string) {
      ...
      if (!tresc || !wybranaGrupa || !kursant) return;
      const zjazd = tematy.find(z => z.tematy === temat);
      if (!zjazd) return;
      setWysylanie(temat);
      ...
      setNowePerTemat(prev => ({ ...prev, [temat]: '' }));
      setWysylanie(null);
    }
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
          <h2 className="page-title" style={{ margin: 0 }}>Tablica pytań</h2>
              ...
              return (
                <div key={z.id} style={{
                    ...
                    <div key={p.id}
                      ...
                      {rozwinietePytanie === p.id && (
                        <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }} onClick={e => e.stopPropagation()}>
                          {p.komentarz_prowadzacego && (
                            <div style={{ padding: '8px 10px', background: '#fef9ec', borderRadius: '8px', border: '0.5px solid #e8d4a0', fontSize: '12px', color: 'var(--text)' }}>
                              <div style={{ fontSize: '9px', fontWeight: 700, color: '#a07830', textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: '3px' }}>Komentarz</div>
                              {p.komentarz_prowadzacego}
                            </div>
                          )}
                          <input type="text"
                  ...
                  <div style={{ marginTop: '4px' }}>
                    <textarea
                      value={nowePerTemat[z.tematy] || ''}
                      onChange={e => setNowePerTemat(prev => ({ ...prev, [z.tematy]: e.target.value }))}
                      onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); dodajPytanie(z.tematy); } }}
                      placeholder="+ Dodaj pytanie lub zapowiedź…"
                      rows={2}
                      style={{
                        width: '100%', fontSize: '12px', padding: '8px 10px',
                        border: '0.5px solid var(--border)', borderRadius: '10px',
                        fontFamily: 'Lato, sans-serif', resize: 'none', background: 'white',
                        boxSizing: 'border-box', color: 'var(--text)',
                      }}
                    />
                    {nowePerTemat[z.tematy]?.trim() && (
                      <button onClick={() => dodajPytanie(z.tematy)} disabled={wysylanie === z.tematy} style={{
                        width: '100%', marginTop: '6px', padding: '8px', background: 'var(--brand)',
                        color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer',
                        fontFamily: 'Lato, sans-serif', fontSize: '12px', fontWeight: 600,
                      }}>
                        {wysylanie === z.tematy ? '…' : 'Wyślij'}
                      </button>
                    )}
                  </div>
                </div>
                    ...
                    {zakladkaGrupy === 'ustawienia' && (
                      <div style={{ background: 'white', borderRadius: '14px', border: '0.5px solid var(--border)', padding: '20px 24px' }}>
                        ...
                        {[
                          ...
                          <div>
  <div style={{ fontSize: '10px', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '5px' }}>Typ kursu</div>
  <select defaultValue={(g as any).typ_kursu || ''}
    onChange={async e => { await supabase.from('grupy').update({ typ_kursu: e.target.value || null }).eq('id', g.id); pobierzGrupy(); setKomunikat('Typ kursu zapisany'); }}
    ...
    <option value="akademia">Roczna Akademia Projektowania</option>
    <option value="projektowanie_wnetrz">Kurs Projektowania Wnętrz</option>
    <option value="ogrody">Kurs Projektowania Ogrodów</option>
    <option value="meble">Kurs Projektowania Mebli</option>
  </select>
</div>
                          <div>
          ...
          {maKislist && (
            <div style={{ background: 'white', borderRadius: '16px', padding: '16px', border: '0.5px solid var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                <img src="https://on-arch.pl/wp-content/uploads/2024/07/kislist-logo.webp" alt="Kislist" style={{ height: '36px', width: 'auto', borderRadius: '8px' }} />
                <div>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)' }}>Dostęp studencki Kislist</div>
                  <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>6 miesięcy bezpłatnego dostępu</div>
                </div>
              </div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', lineHeight: 1.7, background: 'var(--bg)', borderRadius: '10px', padding: '10px 12px' }}>
                🎓 W ramach kursu otrzymujesz <strong style={{ color: 'var(--text)' }}>6-miesięczny dostęp studencki</strong> do serwisu kislist.com — narzędzia do tworzenia profesjonalnych list zakupowych dla klientów.<br/><br/>
                📩 Link aktywacyjny zostanie wysłany na Twój adres email <strong style={{ color: 'var(--text)' }}>pod koniec modułu SketchUp</strong>. Dostęp jest ważny przez 6 miesięcy od momentu aktywacji.
              </div>
              <a href="https://kislist.com" target="_blank" rel="noopener noreferrer"
                style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', marginTop: '10px', fontSize: '12px', color: 'var(--brand)', fontWeight: 500, textDecoration: 'none' }}>
                Dowiedz się więcej o Kislist →
              </a>
            </div>
          )}
            ...
            <div style={{ background: 'white', borderRadius: '16px', padding: '16px', border: '0.5px solid var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                ...
                <div>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)' }}>Świadectwo WSNS Pedagogium</div>
